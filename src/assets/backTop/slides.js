jQuery(document).ready(function($){
	var sHeight = $("#slider_name").height();
	var len = $("#slider_name .silder_panel").length;
	var index = 0;
	var picTimer;

	$(".silder_nav li").css({"opacity":"0.6","filter":"alpha(opacity=60)"}).mouseenter(function() {																		
	...
	// 
	$("#slider_name .silder_con").css("height",sHeight * (len));
	...
	function showPics(index) {
		var Top = -index*sHeight; 
		$("#slider_name .silder_con").stop(true,false).animate({"top":Top},300);
		$(".silder_nav li").removeClass("current").eq(index).addClass("current"); 
		$(".silder_nav li").stop(true,false).animate({"opacity":"0.5"},300).eq(index).stop(true,false).animate({"opacity":"1"},300);
